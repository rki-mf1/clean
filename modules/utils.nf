  ...
  if ( params.keep ) {
    publishDir ( 
      ...
      saveAs: { fn ->
            ...
            fn.endsWith('.fastq.clean.fastq.gz') ? "clean/${fn}".replaceAll(~'.fastq.clean.fastq.gz$', '.fastq.gz') :
            fn.endsWith('.fastq.contamination.fastq.gz') ? "removed/${fn}".replaceAll(~'.fastq.contamination.fastq.gz$', '.fastq.gz') :
            fn
      }
    )
  }
